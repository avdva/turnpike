...
func NewWebsocketServer(realms map[string]Realm) (*WebsocketServer, error) {
	logger.Println("NewWebsocketServer")
	r := NewDefaultRouter()
...
func NewBasicWebsocketServer(uri string) *WebsocketServer {
	logger.Println("NewBasicWebsocketServer")
...
func (s *WebsocketServer) RegisterProtocol(proto string, payloadType int, serializer Serializer) error {
	logger.Println("RegisterProtocol:", proto)
...
func (s *WebsocketServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	logger.Println("WebsocketServer.ServeHTTP", r.Method, r.RequestURI)
	// TODO: subprotocol?
	conn, err := s.Upgrader.Upgrade(w, r, nil)
	if err != nil {
		logger.Println("Error upgrading to websocket connection:", err)
